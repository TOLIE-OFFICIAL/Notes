...
# [JS闭包](https://www.bilibili.com/video/BV1iE411q7Qd/?spm_id_from=333.788)
...
![img](http://pic.tolie.biz/images/202203041752375.gif)
...
> * 浏览器的JS执行引擎`总是访问栈顶的执行上下文`。
> * 全局上下文只有`唯一`的一个，它在浏览器关闭时出栈。
...
<img src="http://pic.tolie.biz/images/202203041752355.png" alt="img" style="zoom:150%;" />
...
![作用域链](http://pic.tolie.biz/images/202203041744767.png)
...
- 创建全局执行上下文；
- 进入第一轮循环，setTimeout将匿名函数放入异步执行对列但不执行，即，将i++放入异步执行队列；
- 下一轮循环执行相同的内容，直至五轮循环结束；
- 执行console.log(i)；执行队列执行结束；
- 执行队列执行结束之后，执行异步队列任务；
- 为匿名函数创建执行上下文；
- 异步队列执行完毕，执行结束；

![作用域链](http://pic.tolie.biz/images/202203041858930.png)
